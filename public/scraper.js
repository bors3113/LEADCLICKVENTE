...
function formatSearchQuery(query) {
  ...
  async function scrapeCurrentView(page) {
    ...
    async function processBatch(batch) {
        // Create an array of promises, one for each result
        const promises = batch.map(async (result) => {
            // Individual stop check will still be honored inside each promise
            ...
            try {
                ...
                await new Promise(resolve => setTimeout(resolve, 1000 + Math.random() * 1000)); // 1-2 seconds instead of 2-5
                ...
                // Give page less time to stabilize
                await newPage.evaluate(() => new Promise(resolve => setTimeout(resolve, 800))); // Reduced from 1500ms
                
                // Get basic business data from Google Maps
                const additionalData = await scrapeGoogleMapsData(newPage);
                Object.assign(result, additionalData);
                
                // Extract contact information from website if available
                if (additionalData.website && additionalData.website !== "N/A") {
                    try {
                        console.log(`Visiting website: ${additionalData.website} to extract contact info`);
                        await newPage.goto(additionalData.website, { 
                            waitUntil: 'domcontentloaded', // Changed from networkidle2 for faster loading
                            timeout: 20000 // Reduced timeout
                        });
                        
                        // Reduced wait time
                        await newPage.evaluate(() => new Promise(resolve => setTimeout(resolve, 1000))); // Reduced from 2000ms
                        
                        // Extract contact information
                        const contactInfo = await extractContactInfo(newPage);
                        result.contactInfo = contactInfo;
                        
                        // Log found contact information
                ...
                // Increment counter when a business is successfully processed
                ...
                // Add empty contact info for failed scrapes
                result.contactInfo = {
                    emails: [],
                    socialMedia: {
                        facebook: [],
...
async function scrapeGoogleMapsData(page) {
    try {
        // Optimize: Extract all data at once to minimize browser interactions
        const data = await page.evaluate(() => {
            // Helper functions for clean extraction
            const getText = (selector) => document.querySelector(selector)?.textContent.trim() ?? "N/A";
            
            // Combine all data extraction in a single operation
            return {
                name: getText('h1.DUwDvf.lfPIob'),
                rating: getText('div.F7nice > span > span[aria-hidden="true"]'),
                reviewsCount: getText('div.F7nice span[aria-label*="avis"]'),
                address: getText('button[data-item-id="address"] div.Io6YTe.fontBodyMedium'),
                website: document.querySelector('a[data-item-id="authority"]')?.href || "N/A",
                phone: getText('button[data-item-id^="phone:tel:"] div.Io6YTe.fontBodyMedium'),
                openingHours: getText('div.OqCZI[jslog] div.MkV9 span.ZDu9vd')
            };
        });

        // Post-process data outside the browser context for better performance
        if (data.rating !== "N/A") {
            data.rating = parseFloat(data.rating.replace(',', '.'));
        }
        
        if (data.reviewsCount !== "N/A") {
            data.reviewsCount = parseInt(data.reviewsCount.replace(/\D/g, ''));
        }

        return data;
    } catch (error) {
        console.error("Error scraping detailed data:", error);
        // Ensure all fields are returned even in case of error
        return {
            name: "N/A",
            rating: "N/A",
            reviewsCount: "N/A",
    ...
    async function extractContactInfo(page) {
        try {
            // Optimize: Extract all data at once with better regexes
            return await page.evaluate(() => {
                // More efficient email regex
                const emailRegex = /([a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.[a-zA-Z0-9-]{2,})/gi;
                
                // More precise social media regexes (only main patterns)
                ...
                // Get all text including hidden elements for better coverage
                const pageContent = document.documentElement.innerHTML;
                
                // Batch process all patterns at once
                const emails = [...new Set(pageContent.match(emailRegex) || [])];
                const socialMedia = {};
                
                // Extract all social media in one go
